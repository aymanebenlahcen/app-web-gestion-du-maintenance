...
if(isset($_POST['date_debut']))
{
  ...
  if(getCount("reparation","where matr_vihecule='$matr_vihecule' and status=0")<=0){
      if($matr_vihecule>0 || $id_entretien>0){
         $query = "INSERT INTO reparation(date_debut,matr_vihecule,id_entretien,status) VALUES('$date_debut','$matr_vihecule','$id_entretien','0')";
         $stmt = $cnx->prepare($query);
          if($stmt->execute())
          {
           
               echo " <script>alert('Reparation Bien Ajouter');</script>";
               echo " <script>
                              window.open('?page=AjouterReparation','_self');
                            </script>";

          }else{
                echo " <script>alert('Reparation Non Ajouter');</script>";
                echo " <script>
                              window.open('?page=AjouterReparation','_self');
                            </script>";
          }

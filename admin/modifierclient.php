...
if (isset($_GET['idclient'])) {
    $idvoitrue = $_GET['idclient'];
    $req = $cnx->query("select * from client where id_client = $idclient");
    $ligne = $req->fetch();
}
?>
...
<div class="container-fluid px-1 py-5 mx-auto">
    <div class="row d-flex justify-content-center">
        <div class="col-xl-7 col-lg-8 col-md-9 col-11 text-center">
            <h2>Modifier le client N°:<?php echo $idclient ?></h2>
            <div class="card">
                <form action="" method="post">
                    ...
                    <div class="row justify-content-between text-left">
                        ...
                        <div class="form-group col-sm-6 flex-column d-flex">
                            ...
                            <select class="form-control" id="type" name="type" value="<?php echo $ligne['type'] ?>" required>
                                <option>PA</option>
                                <option>SO</option>
                            </select>
                        </div>
                ...
                if (isset($_POST['modifier'])) {
                    ...
                    $email = $_POST['email'];
                    $type = $_POST['type'];
                    $req = $cnx->prepare("UPDATE client SET nom = ? , prenom = ? , ville = ? , tel = ? , email = ? , type = ? WHERE id_client = $idclient");
                    $req->execute(array($nom, $prenom, $ville, $tel, $email, $type));
                }
                ?>
            </div>
        </div>

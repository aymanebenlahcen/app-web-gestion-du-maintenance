...
if (isset($_GET['idemployeur'])) {
    $idvoitrue = $_GET['idemployeur'];
    $req = $cnx->query("select * from employeur where id_employeur = $idemployeur");
    $ligne = $req->fetch();
}
?>
...
<div class="container-fluid px-1 py-5 mx-auto">
    <div class="row d-flex justify-content-center">
        <div class="col-xl-7 col-lg-8 col-md-9 col-11 text-center">
            <h2>Modifier l'employeur N°:<?php echo $idemployeur ?></h2>
            <div class="card">
                <form action="" method="post">
                    <div class="row justify-content-between text-left">
                        ...
                        <div class="form-group col-sm-6 flex-column d-flex">
                            ...
                            <input type="email" id="email" name="email" placeholder="" value="<?php echo $ligne['email'] ?>"required>
                        </div>
                    </div>
                    <div class="row justify-content-between text-left">
                        <div class="form-group col-sm-6 flex-column d-flex">
                            <label class="form-control-label px-3">Adresse<span class="text-danger"> *</span></label>
                            <input type="text" id="adresse" name="adresse" value="<?php echo $ligne['adresse'] ?>"required>
                        </div>
                        <div class="form-group col-sm-6 flex-column d-flex">
                            <label class="form-control-label px-3">Date de naissance<span class="text-danger"> *</span></label>
                            <input type="date" id="date_nais" name="date_nais" value="<?php echo $ligne['date_nais'] ?>"required>
                        </div>
                    </div>
                    <div class="row justify-content-between text-left">
                        <div class="form-group col-sm-6 flex-column d-flex">
                            <label class="form-control-label px-3">Role d'employeur<span class="text-danger"> *</span></label>
                            <select class="form-control" id="role" onblur="validate(5)" name="role" value="<?php echo $ligne['role'] ?>">
                ...
                if (isset($_POST['modifier'])) {
                    ...
                    $role = $_POST['role'];
                    $req = $cnx->prepare("UPDATE employeur SET nom = ? , prenom = ? , tel = ? , email = ? , adresse = ? , date_nais = ? , role = ? WHERE id_employeur = $idemployeur");
                    $req->execute(array($nom, $prenom, $tel, $email, $adresse, $date_nais, $role));
                }
                ?>
            </div>
        </div>
